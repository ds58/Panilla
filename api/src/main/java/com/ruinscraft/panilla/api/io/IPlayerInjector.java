...
public interface IPlayerInjector {
    ...
    default String getMinecraftChannelName() {
    ...
    default void register(IPanilla panilla, IPanillaPlayer player) {
        ...
        if (channel.pipeline().get(getMinecraftChannelName()) == null) return;
        if (channel.pipeline().get(CHANNEL_HANDLER_PANILLA) != null) return;
        channel.pipeline().addBefore(getMinecraftChannelName(), CHANNEL_HANDLER_PANILLA, new PanillaChannelHandler(panilla, player));
